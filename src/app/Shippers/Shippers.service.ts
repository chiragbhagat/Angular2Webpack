...
export class ShippersService {
    ...
    getAll() : Observable<ShippersData[]> {
		let url = this.baseUrl + 'api/Shippers';
    ...
    getAllBy(filterExpression: string) : Observable<ShippersData[]> {
		let url = this.baseUrl + 'api/Shippers?filterExpression=${filterExpression}';
    ...
    getAllByPaging(filterExpression: string, pageIndex: number, pageSize: number) : Observable<any> {
		let url = this.baseUrl + `api/Shippers?filterExpression=&sortExpression=ShipperID&pageIndex=${pageIndex}&pageSize=${pageSize}`;
	...
	getByID(id: string): Observable<ShippersData> {
		let url = `${this.baseUrl}api/Shippers/${id}`;
    ...
    addShippersData(body:ShippersData) : Observable<ShippersData> {
    ...
    updateShippersData(body:ShippersData) : Observable<ShippersData> {
		let bodyString = JSON.stringify(body); // Stringify payload
    ...
    deleteShippers(id:string) : Observable<Object>{
		let url = `${this.baseUrl}api/Shippers/${id}`;
...
/*
    function mapShippers(response:Response): ShippersData[] {
        // The response of the API has a results
        // property with the actual results
        return response.json().map(toShippers);
    }

    function toShippers(r:any): ShippersData{
        let region = <ShippersData>({
            RegionID: r.RegionID,
